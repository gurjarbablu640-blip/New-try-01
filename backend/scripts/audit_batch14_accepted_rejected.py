"""Audits accepted and rejected candidates from Batch 14 live autonomous research."""
import json
import os
import random

audit_file = os.path.join(os.path.dirname(__file__), "..", "data", "runtime_state", "batch_14_audit_results.json")
...
rejected = [r for r in results if r.get("status") != "QUALIFIED"]

print("================================================================")
print("BATCH 14 AUDIT REPORT (50 NEW COMPANIES)")
print("================================================================")
...
for r in accepted:
    ...
    print(f"  Facility: {fac.get('name')} in {fac.get('city')} (Linkage: {fac.get('linkage')})")
    print(f"  Person: {pers.get('name')} | Title: {pers.get('title')} (Authority: {pers.get('authority_class')}, Ownership: {pers.get('ownership_score')})")
    print(f"  Lead Score: {r.get('lead_score')} | Apollo Priority: {r.get('apollo_priority')} | Status: {r.get('status')}\n")

print("--- RANDOM REJECTED CANDIDATES AUDIT (2 SAMPLES) ---")
random.seed(42)
sampled_rejected = random.sample(rejected, min(2, len(rejected)))
