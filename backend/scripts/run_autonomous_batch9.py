"""Autonomous Batch 9 Live Research Runner with Recency Gate & Optimized Trigger Discovery.

Evaluates 50 NEW Indian industrial manufacturing accounts (zero overlap with Batches 25, 50, 100, 3, 4, 5, 6, 7, 8).
...
logger = logging.getLogger("batch9_runner")
...
def run_batch9() -> Dict[str, Any]:
    start_time = time.time()
    batch_start_iso = datetime.now(timezone.utc).isoformat()
    logger.info("=== STARTING BATCH 9 LIVE AUTONOMOUS RESEARCH BATCH ===")
    logger.info(f"Target count: {len(BATCH_9_COMPANIES)} new accounts. Reference Date: 2026-09-12")
    ...
    telemetry = {
        "batch_size": len(BATCH_9_COMPANIES),
    ...
    for idx, target in enumerate(BATCH_9_COMPANIES, 1):
        c_name = target["name"]
        c_domain = target.get("domain", "")
        c_sector = target.get("sector", "")
        telemetry["companies_considered"] += 1
        logger.info(f"[{idx}/{len(BATCH_9_COMPANIES)}] Researching {c_name} ({c_sector})...")
    ...
    telemetry["avg_sec_per_company"] = round(total_wall / max(1, len(BATCH_9_COMPANIES)), 2)
    ...
    out_file = os.path.join(out_dir, "batch_9_audit_results.json")
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2)

    logger.info("=== BATCH 9 RESEARCH COMPLETE ===")
