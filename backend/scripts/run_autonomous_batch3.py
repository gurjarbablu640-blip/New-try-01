"""Autonomous Batch 3 Live Research Runner with Recency Gate & Optimized Trigger Discovery.

Evaluates 52 NEW Indian industrial manufacturing accounts (zero overlap with Batches 25, 50, 100).
Implements the largest failure category optimization (multi-query exact-quoted capex discovery).
...
logger = logging.getLogger("batch3_runner")
...
def run_batch3() -> Dict[str, Any]:
    start_time = time.time()
    batch_start_iso = datetime.now(timezone.utc).isoformat()
    logger.info("=== STARTING BATCH 3 LIVE AUTONOMOUS RESEARCH BATCH ===")
    logger.info(f"Target count: {len(BATCH_3_COMPANIES)} new accounts. Reference Date: 2026-09-12")
    ...
    telemetry = {
        "batch_size": len(BATCH_3_COMPANIES),
    ...
    for idx, target in enumerate(BATCH_3_COMPANIES, 1):
        c_name = target["name"]
        c_domain = target.get("domain", "")
        c_sector = target.get("sector", "")
        telemetry["companies_considered"] += 1
        logger.info(f"[{idx}/{len(BATCH_3_COMPANIES)}] Researching {c_name} ({c_sector})...")
    ...
    telemetry["avg_sec_per_company"] = round(total_wall / max(1, len(BATCH_3_COMPANIES)), 2)
    ...
    out_file = os.path.join(out_dir, "batch_3_audit_results.json")
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2)

    logger.info("=== BATCH 3 RESEARCH COMPLETE ===")
