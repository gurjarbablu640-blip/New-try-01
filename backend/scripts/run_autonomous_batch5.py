"""Autonomous Batch 5 Live Research Runner with Recency Gate & Optimized Trigger Discovery.

Evaluates 51 NEW Indian industrial manufacturing accounts (zero overlap with Batches 25, 50, 100, 3, 4).
...
logger = logging.getLogger("batch5_runner")
...
def run_batch5() -> Dict[str, Any]:
    start_time = time.time()
    batch_start_iso = datetime.now(timezone.utc).isoformat()
    logger.info("=== STARTING BATCH 5 LIVE AUTONOMOUS RESEARCH BATCH ===")
    logger.info(f"Target count: {len(BATCH_5_COMPANIES)} new accounts. Reference Date: 2026-09-12")
    ...
    telemetry = {
        "batch_size": len(BATCH_5_COMPANIES),
    ...
    for idx, target in enumerate(BATCH_5_COMPANIES, 1):
        c_name = target["name"]
        c_domain = target.get("domain", "")
        c_sector = target.get("sector", "")
        telemetry["companies_considered"] += 1
        logger.info(f"[{idx}/{len(BATCH_5_COMPANIES)}] Researching {c_name} ({c_sector})...")
    ...
    telemetry["avg_sec_per_company"] = round(total_wall / max(1, len(BATCH_5_COMPANIES)), 2)
    ...
    out_file = os.path.join(out_dir, "batch_5_audit_results.json")
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2)

    logger.info("=== BATCH 5 RESEARCH COMPLETE ===")
